...
func getAccessToken(ctx *gin.Context, req *oauth2Req) *tokenRes {
	token := new(tokenRes)
	//
	...
	//
	url := fmt.Sprintf("%s/token", oauth2Host)
	err := util.HTTP[int](http.MethodPost, url, query, nil, token, http.StatusOK, apiCallTimeout)
	if err != nil {
